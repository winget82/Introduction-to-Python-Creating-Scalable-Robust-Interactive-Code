...
def parse_command_line():
    ...
    parser.add_argument('-d', '--date', default=[], nargs = 3, metavar = ("month", "day", "year"), help = 'formatted date (i.e. 03 28 1998)')
    parser.add_argument('-s', '--surprise', action = 'store_true', help = 'select a random date for a surprise image')
    parser.add_argument('-k', '--api_key', type=str, default='', help = 'NASA developer key')
...
def create_date(datelist, surprise):
    ...
    """
    #MUST UTILIZE - datetime.today(), timedelta(), try, except, return
    ...
    begin_date = date(month = 6, day = 16, year = 1995)

    try:
    #make a date object

        if datelist != []:
            datelist = date(month = int(datelist[0]), day = int(datelist[1]), year = int(datelist[2]))

            if (datelist <= today2) and (datelist > begin_date):
                #datelist = date(month = int(datelist[0]), day = int(datelist[1]), year = int(datelist[2]))
                print(datelist, 'date object working correctly')
                return datelist

            elif (datelist >= (today2 + delta1)) or (datelist < begin_date):
                return None

        #random date between June 16 1995 and today - surprise
        elif (datelist == []) and (surprise == True):
            rand_y = randint(1995,2018)
            rand_m = randint(1,12)
            rand_d = randint(1,31)
            rand_date = date(month = rand_m, day = rand_d, year = rand_y)
            if (rand_date >= begin_date):
                surprise = rand_date
                return surprise

        #get yesterday
        elif (datelist == []) and (surprise == False):
            return yesterday
...
def query_url(d, api_key):
    ...
    """
    print('your api = ' + str(api_key))
    #TODO: Your code goes here
    baseURL = 'https://api.nasa.gov/planetary/apod?'
    api_key_formatted = 'api_key=' + str(api_key)

    #d2 = date(year = d[2], month = d[0], day = d[1])
    date_formatted = d.strftime('&date=%Y-%m-%d')
    url = str(baseURL) + str(api_key_formatted) + str(date_formatted)
    return url
...
def save_image(d, image):
    ...
    #TODO: Your code goes here
    date_formatted = str(d.strftime('%Y-%m-%d'))
    filename = str(date_formatted + '.jpg')

    folder_month = str(d.month)
    folder_year = str(d.year)


    if (folder_year not in os.getcwd()):
        if os.path.exists("./" + folder_year):
            print("Changing working dir to " + folder_year)
            os.chdir(folder_year)

            if os.path.exists("./" + folder_month):
                print("Changing working dir to " + folder_month)
                os.chdir(folder_month)

            else:
                os.mkdir(os.getcwd() + "/" + folder_month)
                print("Changing working dir to " + folder_month)
                os.chdir(folder_month)
                file_path = os.path.abspath(folder_month) + str(filename)

                with open(filename, 'wb') as binary_image:
                    binary_image.write(image)
                    
                return file_path

        else:
            os.mkdir(os.getcwd() + "/" + folder_year)
            print("Changing working dir to " + folder_year)
            os.chdir(folder_year)
...
def main():
    ...
    API_KEY = "get your own key and put it here"
    #API_KEY = "nU9TZhXXWdq15UF9yyUA5hSwnTtSZONOyFeJREYe"
